...
if (isset($_SESSION['project_user_id'])){
  ...
  if(isset($_GET["finance"])){ if(!empty($_GET["finance"])){ if($WHERE_QUERY != 'WHERE'){ $WHERE_QUERY .= " AND"; } $WHERE_QUERY .= " `finance_approve` =  '" . $_GET["type"] . "'"; }}
  if(isset($_GET["department"])){ if(!empty($_GET["department"])){ if($WHERE_QUERY != 'WHERE'){ $WHERE_QUERY .= " AND"; } $WHERE_QUERY .= " `status` >= '" . $_GET["type"] . "'"; }}
...
?>
      <?php
               ...
               while($row=mysqli_fetch_array($col)){
                ...
                function nullcheck($data){if (is_null($data)) { return ''; }else{ return $data; }}
      ?>
                                            <tr>
                                                <td><?= $row['id'] ?></td>
                                                ...
                                                <td class="text-center"><?= nullcheck($row['department_approve_by']) ?></td>
                                                <td class="text-center"><?= nullcheck($row['department_approve_on']) ?></td>

                                            <?php if($row['status'] == 'Approved'){ ?>
                                                <td class="text-center"><?= statusbadge($row['finance_approve']); ?></td>
                                            <?php }else{ ?>
                                                <td class="text-center"></td>';
                                            <?php } ?>

                                                <td class="text-center"><?= nullcheck($row['finance_approve_by']) ?></td>
                                                <td class="text-center"><?= nullcheck($row['finance_approve_on']) ?></td>

                                            <?php if($row['issued_littres'] > 0){ ?>
                                                <td class="text-center"><span class="badge bg-success">Issued</span></td>';
                                            <?php }else{ ?>
                                                <td class="text-center"></td>';
                                            <?php } ?>
                                                <td class="text-center"><?= nullcheck($row['issued_by']) ?></td>
                                                <td class="text-center"><?= nullcheck($row['issued_littres']) ?></td>
                                                <td class="text-center"><?= nullcheck($row['issued_date']) ?></td>
                                                <td class="NoneDisplay" width="150" style="text-align:center"><a class="btn btn-sm btn-info EDIT-DATA" style="color:#fff;"><span style="display:none;"><?php echo (json_encode($row)); ?></span><i class="fa fa-eye"></i></a>
                                            </tr>
