                            ...
                            <div class="card-content">
                                <div class="table-responsive">
                                    <table class="table mb-0">
                                        ...
                                        <tbody>
<?php
        if (mysqli_query($link,"SELECT `id` FROM `fc` LIMIT 1")->num_rows > 0) {
?>
      <?php
              ...
              $statement = "`fc` LEFT OUTER JOIN `psf` ON psf.`id` = fc.`psf_id`"; //you have to pass your query over here

              $col=mysqli_query($link,"SELECT fc.*,psf.`customer_name`, DATE(fc.`datetime`) AS `datetime` FROM {$statement} ORDER BY `id` DESC LIMIT {$startpoint} , {$limit}");
      ...
      ?>
                                          <tr>
                                            ...
                                            <td class="text-center"><?= status_check($row['marketing_approve']); ?></td>
                                            <td class="NoneDisplay" width="150" style="text-align:center">
                                              <a class="btn btn-sm btn-info" style="color:#fff;" href="marketing_fc?fr=<?= $row['fr_id'] ?>"><i class="fa fa-eye"></i></a>
                                            </td>
                                          </tr>
