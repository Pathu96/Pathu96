...
<html class="loading" lang="en" data-textdirection="ltr">
  <head>
    <title>Formula Receipt History | Product Approval</title>
        ...
        <div class="content-header row">
          <div class="content-header-left col-md-12 col-12 mb-2">
            <div class="row breadcrumbs-top">
              <div class="col-12">
                <h2 class="content-header-title text-center mb-0">Formula Receipt History</h2>
              </div>
            </div>
          </div>
        </div>
                                    ...
                                    <table class="table mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-center">Finance Approval</th>
                                                <th class="text-center">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
<?php
        if (mysqli_query($link,"SELECT `id` FROM `fr` WHERE fr.`qa_approve` = 'Approved' OR fr.`qa_approve` = 'Rejected' LIMIT 1")->num_rows > 0) {
?>
      <?php
              ...
              $statement = "`fr` LEFT OUTER JOIN `psf` ON psf.`id` = fr.`psf_id` WHERE fr.`qa_approve` = 'Approved' OR fr.`qa_approve` = 'Rejected' "; //you have to pass your query over here
      ...
      ?>
                                          <tr>
                                            ...
                                            <td class="text-center"><?php if($row['qa_approve'] == "Approved"){ echo(status_check($row['finance_approve'])); } ?></td>
                                            <td class="NoneDisplay" width="150" style="text-align:center">
                                              <a class="btn btn-sm btn-info" style="color:#fff;" href="qa_fr?psf=<?= $row['psf_id'] ?>"><i class="fa fa-eye"></i></a>
                                            </td>
                                          </tr>
    ...
    <script type="text/javascript">
      function exportReportToExcel() {
        ...
        TableToExcel.convert(table[1], { // html code may contain multiple tables so here we are refering to 1st table tag
          name: `Formula Receipt History.xlsx`, // fileName you could use any name
          sheet: {
